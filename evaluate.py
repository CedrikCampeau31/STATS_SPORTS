...
RENAME = {
    ...
    "ES":"es_pts","PP":"pp_pts","SH":"sh_pts","ESG":"esg","PPG":"ppg","SHG":"shg",
    "GWG":"gwg","OTG":"otg","ESA":"esa","PPA":"ppa","SHA":"sha","GWA":"gwa","OTA":"ota",
    "ESP":"esp","PPP":"ppp","SHP":"shp","GWP":"gwp","OTP":"otp","PPP%":"ppp_pct",
    "G/60":"g_per60","A/60":"a_per60","P/60":"p_per60","ESG/60":"esg_per60",
    "ESA/60":"esa_per60","ESP/60":"esp_per60","PPG/60":"ppg_per60","PPA/60":"ppa_per60",
    "PPP/60":"ppp_per60","G/GP":"g_pergp","A/GP":"a_pergp","P/GP":"p_pergp",
    "SHOTS":"shots","SH%":"sh_pct","HITS":"hits","BS":"blocks","FOW":"fow","FOL":"fol","FO%":"fo_pct",
    "Season":"seasonId",
}

KEEP = [
    "rank","playerName","team","age","pos","seasonId","gp","g","a","pts","pim","pm","toi",
    "es_pts","pp_pts","sh_pts","esg","ppg","shg","gwg","otg","esa","ppa","sha","gwa","ota",
    "esp","ppp","shp","gwp","otp","ppp_pct","g_per60","a_per60","p_per60","esg_per60","esa_per60",
    "esp_per60","ppg_per60","ppa_per60","ppp_per60","g_pergp","a_pergp","p_pergp",
...
def toi_to_minutes(x):
    ...
    if re.match(r"^\d{1,2}:\d{2}:\d{2}$", s):
        h, m, sec = s.split(":"); return int(h)*60 + int(m) + int(sec)/60.0
...
def build_tminus1_to_t(df):
    base_cols = [c for c in [
        ...
        "gp","g","a","pts","pim","pm","es_pts","pp_pts","sh_pts",
        "esg","ppg","shg","gwg","otg","esa","ppa","sha","gwa","ota",
        "esp","ppp","shp","gwp","otp","ppp_pct",
        "g_per60","a_per60","p_per60","esg_per60","esa_per60","esp_per60",
        "ppg_per60","ppa_per60","ppp_per60","g_pergp","a_pergp","p_pergp",
        "shots","sh_pct","hits","blocks","fow","fol","fo_pct","toi_min_total"
    ] if c in df.columns]
...
def run_training(df):
    ...
    for mp in ["FWD","DEF"]:
        ...
        if k == 1:
            tier_models[mp] = None; tier_centers[mp] = np.array([np.nan]); continue
        km = KMeans(n_clusters=k, random_state=RANDOM_STATE, n_init=10).fit(pts_train.reshape(-1,1))
        tier_models[mp] = km; tier_centers[mp] = km.cluster_centers_.flatten()
    ...
    models[("GLOBAL","_")] = HistGradientBoostingRegressor(random_state=RANDOM_STATE).fit(X_tr, y_tr)
    ...
    def export_eval(mask, split_name):
        ...
        sub[preds_cols].to_csv(os.path.join(BASE_MODELS, f"predictions_{split_name}.csv"), index=False)
        pd.DataFrame([{"split":split_name, **m}]).to_csv(os.path.join(BASE_MODELS, f"metrics_{split_name}.csv"), index=False)
    ...
    sub_next[out_cols + ["seasonId_pred"]].to_csv(os.path.join(BASE_MODELS, f"predictions_next_{next_season}.csv"), index=False)

    # results.json pour CI
    results = {
        "val": MET_VAL,
        "test": MET_TEST,
        "splits": {"next": int(next_season)}
    }
